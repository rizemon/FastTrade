...
<body>
    ...
    <?php
        ...
        select srcUID, content, time, username, picture from chat join userinfo
        on chat.srcUID = userinfo.UID
        where chat.destUID = ? and msgID in
        (select MAX(msgID) from chat
        group by srcUID)
        order by time desc
        ");
        $stmt->bind_param("i", $srcUID);
        $stmt->execute();
